...
dfa::dfa() {
	...
	for (int i = 0; i < finalStates.size(); i++)
		in >> finalStates[i];
	in >> size;
	transitFunction.resize(size);
	for (int i = 0; i < transitFunction.size(); i++) {
		transitFunction[i].resize(alphabet.size());
		for (int j = 0; j < transitFunction[i].size(); j++)
			in >> transitFunction[i][j];
	}
}

dfa::dfa(const dfa& new_dfa) {
	this->countStates = new_dfa.countStates;
	this->alphabet = new_dfa.alphabet;
	this->finalStates = new_dfa.finalStates;
	this->transitFunction.resize(new_dfa.countStates);
	for (int i = 0; i < new_dfa.countStates; i++)
		this->transitFunction[i] = new_dfa.transitFunction[i];
}
...
bool dfa::isAccept(std::string s, int& state)
{
	state = 0;
	for (char c : s)
	{
...
void dfa::printf() {
	...
	for (int i = 0; i < finalStates.size(); i++)
		out << finalStates[i] << " ";
	out << "\n";
	out << transitFunction.size() << "\n";
	for (int i = 0; i < transitFunction.size(); i++) {
		for (int j = 0; j < transitFunction[i].size(); j++)
			out << transitFunction[i][j] << " ";
		out << "\n";
	}
}